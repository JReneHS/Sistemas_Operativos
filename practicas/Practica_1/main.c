...
/* manejo de procesos hijos */
  for (int i = 1; i < argc; i++)//Ciclo de creacion de hijos mediante fork()
  {
    if ((pid = fork()) == 0)//validacion de proceso hijo para ejecutar su parte de codigo
    {
      ...
      execv("fibonacci",args);//reemplaza el codigo del proceso que está corriendo y manda un vector de argumentos.
    }
    else
    {
  ...
  for (int i = 1; i < argc; i++)//ciclo de wait() para esperar a todos los hijos creados
  {
    wait(NULL);
  }
